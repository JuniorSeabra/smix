...
export class JwtStrategy extends PassportStrategy(Strategy) {
  ...
  async validate(payload: JwtPayload) {
    ...
    const user = await this.prisma.user.findUnique({
      ...
      select: { id: true, status: true, role: true, accessExpiresAt: true },
    });
    if (!user || user.status !== 'ACTIVE') {
      throw new UnauthorizedException('Conta desativada ou não encontrada');
    }
    // Data de expiração de acesso definida pelo admin — checada a cada
    // requisição pelo mesmo motivo do status: precisa derrubar a sessão na
    // hora, não só bloquear um próximo login.
    if (user.accessExpiresAt && user.accessExpiresAt.getTime() <= Date.now()) {
      throw new UnauthorizedException('Seu acesso expirou. Fale com o administrador.');
    }
